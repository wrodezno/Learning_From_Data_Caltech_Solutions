...
#Purpose: CalTech Learning From Data, Final Problem 16_17_18
#Data: 12/5/2013
...
RBF_Ein1_5 = np.zeros(exp_length)
RBF_Eout1_5 = np.zeros(exp_length)
RBF_Ein2 = np.zeros(exp_length)
RBF_Eout2 = np.zeros(exp_length)
...
#Performin RBF as a kernal for support vector machines
    ...
    for r in rho:
        ...
        #and a ones column 
        Qin = Qmatrix(centers,K,x,[0,N],r)


        #Pseudo-inverse to obtain rbf weights
        Pseudo_inverse = inv(dot(Qin.transpose(),Qin))
        w = dot(dot(Pseudo_inverse,Qin.transpose()),y)
        #Obtaining Eout and Ein for Regular RBF Functions
        #In-Sample Performance for RBF
        gin = np.sign(np.dot(Qin,w))

        #Out-Of-Sample Performance
        #Obtaining Q matrix for Test Dataset
        Qout = Qmatrix(centers,K,X,[N,Total],r)

        #Eout for RBF
        gout = np.sign(np.dot(Qout,w))
        

        if r == 1.5:
            RBF_Ein1_5[run] = np.sum(gin!=y,dtype='float')/(N)
            RBF_Eout1_5[run] = np.sum(gout!=Y[N:],dtype='float')/(Total-N)
        else:
            RBF_Ein2[run] = np.sum(gin!=y,dtype='float')/(N)
            RBF_Eout2[run] = np.sum(gout!=Y[N:],dtype='float')/(Total-N)            
